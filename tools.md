# The Tools Used in `ng-sass-starter`

## Introduction

`ng-sass-starter` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ng-sass-starter` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ng-sass-starter` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ng-sass-starter`, this project makes the assumption
...
While `ng-sass-starter` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ng-sass-starter` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ng-sass-starter`; Grunt is
...
to get started with `ng-sass-starter`. But for those curious or looking to go a
...
Grunt is the engine behind `ng-sass-starter`. It's the magic that makes it move.
...
`ng-sass-starter` comes with a `bower.json` file that looks something like this:
...
in `ng-sass-starter` by default in there to see what I mean.
